// 🤖 Vendors the docs corpus from a rastrillo checkout into src/docs/.
//
// The corpus lives in rastrilloorg/rastrillo under docs/site, where it
...
if (!checkout) {
  console.error("usage: node hack/sync-docs.mjs <path-to-rastrillo-checkout> [--check]");
  process.exit(2);
}
...
function walk(dir, match = (name) => name.endsWith(".md")) {
  const out = [];
